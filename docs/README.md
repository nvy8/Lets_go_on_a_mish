...
| 2026-05-24 | [changelog/2026-05-24-full-flow-verification.md](changelog/2026-05-24-full-flow-verification.md) | Full end-to-end verification of all 5 stages × happy/worst paths + 8 edge cases + critique loop + auth enforcement on 6 endpoints. Found and fixed Stage 5 empty-picks badge bug; identified intermittent Stage 2 hallucinated-`top_3_ids` flake (not fixed). |
| 2026-05-24 | [changelog/2026-05-24-teacher-students-analytics.md](changelog/2026-05-24-teacher-students-analytics.md) | New "Your class" analytics section on `/teacher/missions/[id]`. API endpoint returns per-student progress + per-class summary; UI renders summary stats + stage distribution + sortable/filterable table + drilldown modal + CSV export. 20s auto-refresh, status dots, hand-drawn styling throughout. |
| 2026-05-24 | [changelog/2026-05-24-projects-grouping-missions.md](changelog/2026-05-24-projects-grouping-missions.md) | New Project entity that optionally groups missions into multi-week units. Backwards compat: existing missions become standalone. New `/api/projects` + `/api/projects/[id]` CRUD, missions API extended with `project_id` + `position` + `week_number`. Dashboard refactored into Projects + Standalone sections, new `/teacher/projects/[id]` page with per-project aggregate stats + week-grouped mission list. Archive + delete (detach, not cascade) semantics. |
