...
export function Stage6({ shareToken }: { shareToken: string }) {
  ...
  useEffect(() => {
    fetch("/api/stage/6/init", { method: "POST" })
      .then((r) => r.json())
      .then((d) => {
  ...
  async function finish() {
    setSubmitting(true);
    try {
      const res = await fetch("/api/stage/6/submit", {
        method: "POST",
    ...
    return (
      <div className="rounded-2xl border border-amber-300 bg-amber-50 p-8 text-center">
        ...
        <h2 className="mt-3 text-xl font-semibold">
          {allPerfect ? "Badge unlocked: Hallucination Hunter" : "Stage 6 complete"}
        </h2>
